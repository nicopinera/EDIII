 ...
 * Tienen un numero de excepciones, un IRQ, una prioridad y pueden ser sincronas o asincronas
 *
 ...
 *	SiAdemas podemos, habilitando la EINT 3, habilitamos las interrupciones por GPIO
 *	Porque comparten el mismo lugar en la tabla.
 *
 */
...
void configGPIO(){
	...
	LPC_PINCON ->PINSEL4 |= (0b01<<26); // Configuro como interrupcion Externa al pin P2.13

	LPC_GPIOINT->IO0IntEnR |= 0b1111; // Configurar interrupciones de P0.0 al P0.4

	LPC_SC->EXTMODE = (0<<3); //Interrumpe por nivel

	NVIC_Enable(EINT3_IRQn); // habilitio la externa 3 para poder configurar los pines
}
