 ...
 * intercalando (secuencia A - secuencia B- secuencia A- ... ). Como todavía no
 * se ha definido la frecuencia a la cual va a funcionar el CPU del
 * microcontrolador, las funciones de retardos que se incorporen deben tener
 * como parámetros de entrada variables que permitan modificar el tiempo de
 * retardo que se vaya a utilizar finalmente. Se pide escribir el código que
 * resuelva este pedido,considerando que los leds se encuentran conectados en
...
void configuracionGPIO() {
  ...
  LPC_GPIO0->FIODIR |=
      (0X3FF); // PONGO LOS 10 PRIMEROS EN '1' PARA PONER SALIDA
