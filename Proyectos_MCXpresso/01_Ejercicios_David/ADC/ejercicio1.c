/* Escriba un programa que configure un canal del ADC para leer un voltaje
 * variable. Pines: Utilice el pin AD0.0 (P0.23) para la entrada analógica
 * (conectado, por ejemplo, a un potenciómetro). Salida: El valor de 12 bits
 * leído debe ser escalado a un valor de 1 bit para encender o apagar un LED. El
 * LED debe encenderse si la lectura supera la mitad del rango (2048).
 *
 * Condiciones:
...
void ADC_IRQHandler(void) {
  uint32_t valor_adc = ADC_ChannelGetData(
      LPC_ADC, 0); // tomo el dato de un canal - ya me lo devuelve >>4 y &0xFFF
