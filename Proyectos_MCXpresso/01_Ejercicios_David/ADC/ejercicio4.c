 ...
 * La adquisición de datos es iniciada por una interrupción externa y cronometrada por un temporizador.
 *
 * Pines:
 ...
 * LED de Estado: Utilice el pin P0.22 para indicar el estado del sistema:
 * 		encendido si el ADC está activo y muestreando, y apagado si está inactivo.
 *
 * Lógica de Muestreo:
 ...
 * Muestreo: Cada evento de match del Timer 1 debe disparar una nueva conversión del ADC.
 * Almacenamiento: El resultado de cada conversión del ADC debe ser leído por el CPU y
 * almacenado secuencialmente en un array de 100 posiciones en la SRAM Bank 1.
 * Finalización Normal: Después de la muestra número 100, el Timer 1 y el ADC deben ser deshabilitados, y el LED de Estado debe apagarse.
 *
 * Lógica de Control (Botón de Pausa/Reinicio):
 * Si se detecta un segundo flanco de bajada en el botón mientras la adquisición está en curso (antes de llegar a 100 muestras):
 ...
 *
 * Condiciones:
 * Asegure que las muestras de 10 bits del ADC se almacenen correctamente en un array de enteros
 * de 16 o 32 bits, garantizando la correcta alineación de los bits de datos (0-11 o 0-12).
 * La gestión de la cuenta y el toggling del estado debe manejarse completamente dentro de la
 * ISR de EINT0 y la ISR del Timer 1.
 *
 * */
