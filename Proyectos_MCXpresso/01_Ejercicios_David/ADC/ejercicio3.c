/* Desarrolle un programa que permita al usuario controlar de forma
 * independiente la intensidad (brillo) de los tres colores (Rojo, Verde, Azul)
 * de un LED RGB, utilizando tres potenciómetros. La modulación de intensidad
 * debe implementarse mediante PWM.
 *
 * Pines:
 * Entradas ADC (Potenciómetros): Utilice tres canales de ADC disponibles, AD0.0
 * (P0.23), AD0.1 (P0.24) y AD0.2 (P0.25). Salidas PWM (LED RGB): Utilice los
 * pines de Match del Timer 2 (que soporta hasta 4 canales de Match) para
 * generar las señales PWM. Asigne MR2.0, MR2.1 y MR2.2 para controlar los pines
 * del LED RGB (Rojo, Verde, Azul).
 *
 *  Lógica:
 *  Configure el ADC para escanear y convertir los tres canales de entrada de
 * forma secuencial. Configure el Timer 2 para una frecuencia PWM fija. El valor
 * de 10 bits de cada lectura de ADC debe ser mapeado y utilizado directamente
 * para establecer el ciclo de trabajo (duty cycle) de la señal PWM
 * correspondiente a cada color. ADC0.0 → Controla el Duty Cycle de PWM (MR2.0)
 * - Color Rojo. ADC0.1 → Controla el Duty Cycle de PWM (MR2.1) - Color Verde.
...
void ADC_IRQHandler(void) {
  ...
  uint32_t valor_final =
      (uint32_t)((valor_adc * TIMER_MATCH_PERIODO) / max_val);
