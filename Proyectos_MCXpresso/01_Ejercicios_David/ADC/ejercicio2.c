/* Escriba un programa que lea tres canales analógicos distintos de forma
 * secuencial. Pines: Utilice los pines AD0.0 (P0.23), AD0.1 (P0.24) y AD0.2
 * (P0.25) como entradas.
 *
 * Lógica:
 * Utilice 4 LEDs para implementar un vúmetro por cada canal, de forma en que se
 * encienda un LED adicional por cada 25% del nivel máximo.
 *
 * Condiciones:
 * Utilice un timer para disparar las conversiones de manera secuencial, con un
 * intervalo entre conversión de 50 ms.
 *
 *
 * */
...
#define FUNC_3 3
#define TIMERMATCH0                                                            \
  250000 // valor del registro match para periodo - periodo de 50 ms - debo
         // cargar la mitad para toggle
...
void ADC_IRQHandler(void) {
  ...
  case 0: // canal 0
    if (valor_final >= 25) {
      GPIO_SetValue(puerto_led[0].puerto,
                    1 << puerto_led[0].pin); // prendo primer led
    }
    if (valor_final >= 50) {
  ...
  case 1: // canal 1
    if (valor_final >= 25) {
      GPIO_SetValue(puerto_led[4].puerto,
                    1 << puerto_led[4].pin); // prendo primer led
    }
    if (valor_final >= 50) {
  ...
  case 2: // canal 2
    if (valor_final >= 25) {
      GPIO_SetValue(puerto_led[8].puerto,
                    1 << puerto_led[8].pin); // prendo primer led
    }
    if (valor_final >= 50) {
