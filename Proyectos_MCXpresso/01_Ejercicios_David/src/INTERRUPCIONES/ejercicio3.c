 ...
 *
 * Pines: Utilice un display de 7 segmentos conectado a un puerto de su elección. Un pulsador,
 * conectado a un pin configurado como interrupción externa (EINT0, EINT1 o EINT2), controlará la dirección.
 *
 * Lógica:
 * Inicialmente, el contador debe avanzar de forma automática (ascendente).
 * Cuando el botón se mantenga presionado, el contador debe detenerse. Al soltar el botón, el
 * sentido de la cuenta debe invertirse (descendente) a partir del valor actual.
