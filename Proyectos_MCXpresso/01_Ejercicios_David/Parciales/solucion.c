...
volatile uint8_t *const ahb_buffer =
    (uint8_t *)AHB_SRAM_BASE; // primeros 256 bytes

volatile uint8_t *const ahb_index =
    (uint8_t *)(AHB_SRAM_BASE +
                BUFFER_SIZE); // un solo valor que voy a actualizar

volatile uint8_t *const ahb_sample_count =
    (uint8_t *)(AHB_SRAM_BASE + BUFFER_SIZE + 1); // 258
volatile uint32_t *const ahb_timestamp =
    (uint32_t *)(AHB_SRAM_BASE + BUFFER_SIZE + 1); // 259
 ...
 */
void config_SysTick(void) { SysTick_Config(SystemCoreClock / 1000); }
