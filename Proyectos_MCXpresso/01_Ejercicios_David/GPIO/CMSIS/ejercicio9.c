/* Utilice dos grupos de 4 pines de entrada (A_in, B_in) y un interruptor para
 * seleccionar una operación. El interruptor (conectado a un pin) debe decidir
 * si los valores de A_in y B_in se suman o se restan. El valor absoluto del
 * resultado debe mostrarse en 4 LEDs de salida. Utilice un LED adicional para
 * mostrar si se genera un overflow en la suma o si el resultado de la resta es
 * negativo
 */
...
int main(void) {
  ...
  while (1) {
    ...
    case 0: // Suma
      resultado =
          A_in + B_in; // se guarda con signo - ultimo bit es el del signo
