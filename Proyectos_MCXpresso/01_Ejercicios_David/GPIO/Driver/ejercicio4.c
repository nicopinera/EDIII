 ...
 * Escriba un programa que identifique la presión de un pulsador conectado a un
 * pin GPIO configurado como entrada. Utiliza otro pin de salida para encender
 * un LED solo mientras el botón está presionado.
 * */
...
Puertos_t puert[] = {
    {0, 0, INTPUT}, // p0.0 -> ENTRADA DEL PULSADOR
...
int main(void) {
  ...
  while (1) {
    ...
    if (puerto0 & 0b1 == 0) {
      GPIO_SetValue(puert[1].puerto,
                    puert[1].pin); // prende por alto (pin - led - gnd)
