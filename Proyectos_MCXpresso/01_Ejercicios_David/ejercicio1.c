...
// Configurar timer para que nos habilite el adc MAT0.1 (100ms) para pedir una muestra
// en funcion de la muestra si es de 0-40 prendo el verde - 41 a 60 para amarillo y si tengo 10 muestras seguidas mayor a 60 grados
// prendo el rojo
...
void configGPIO() {
	...
	for (int i = 0; i < NUMERO_LED; i++) {
		...
		GPIO_SetDir(pin_led[i].puerto, 1 << pin_led[i].pin, 1); // definis como salida
