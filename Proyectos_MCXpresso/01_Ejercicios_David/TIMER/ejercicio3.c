 ...
 * El PWM debe iniciar con un 50% de ciclo de trabajo. Cada vez que se presione
 el botón, el ciclo de trabajo debe incrementarse en un 10%.
 * Al alcanzar el 100%, la siguiente pulsación debe reiniciar el ciclo de
 trabajo a 0%.
 *
 * Condiciones:
 * Utilice el Timer2 para generar la señal PWM.
 * Conecte el botón a un pin GPIO con interrupción externa para controlar el
 incremento del ciclo de trabajo.
 * El periodo de la onda debe ser fijo, mientras que el ancho del pulso debe ser
 ajustable.
...
void EINT0_IRQHandler(void) {
  ...
  TIM_UpdateMatchValue(LPC_TIM0, 1,
                       match1_reg); // actualizo el valor del match en el canal
                                    // 1 que maneja el duty
...
void configTIMER(void) {
  ...
  // canal 0 -> periodo fijo de la onda -> prende el led cuando inicia le
  // periodo
