/* Escriba un programa que utilice un Timer para generar una onda cuadrada de 1
 Hz.
 * Agregue un botón que, al ser presionado, duplique la frecuencia de la onda.
 * La frecuencia máxima debe ser de 1024 Hz, y la siguiente presión del botón
 debe reiniciar la frecuencia a 1 Hz. Condiciones: • Utilice un Timer de su
 elección y configure su canal de Match para generar la onda. • El botón debe
 estar conectado a un pin GPIO configurado con interrupción externa. • La
 frecuencia debe ser modificada en el Handler de la interrupción.
 * */
...
#define FUNC_3 3
#define TIMERMATCH                                                             \
  12500000 // valor del registro match base para 1Hz - interrumpe en medio
           // periodo para hacer el toggle
